...
public final class FusionModelLifecycle {
    ...
    public static void onFusionModifiersApplied(ModelBakery bakery) {
        Map<ModelResourceLocation, BakedModel> authoritative =
        ...
        // 中文：1.21.1 的 state 模型映射是从权威烘焙 map 恢复的视图；只把实际 Fusion
        ...
        // English: The 1.21.1 state-model map is a recovered view of the authoritative baked
        ...
        int verified = 0;
        for (BlockState state : decorated.keySet()) {
            if (authoritative.get(
                    BlockModelShaper.stateToModelLocation(state))
                    instanceof FusionConnectedBlockStateModel) {
                verified++;
            }
        }
        Constants.LOG.info(
                "Installed Fusion-native AutoBlend model routing for {} block states "
                        + "(written back {}, verified {})",
                decorated.size(),
                writtenBack,
                verified);
    }
