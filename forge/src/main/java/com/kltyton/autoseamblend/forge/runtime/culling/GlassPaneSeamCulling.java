package com.kltyton.autoseamblend.forge.runtime.culling;
...
/**
 * 中文：让原版玻璃板端盖 Quad 使用与连接纹理剔除资源包相同的剔除桶（纯 Forge 路径）。
 *
 * English:
 * Gives vanilla pane cap quads the same cull buckets as connected-texture pane-culling
 * resource packs (pure Forge path).
 *
 * <p>中文：1.20.1 Forge 上 CTM/Fusion/Athena 走 vanilla BakedModel#getQuads 渲染路径，
 * 因此包装器只需在 3 参（vanilla）与 5 参（Forge ModelData/RenderType）getQuads 中做
 ...
 * ForgeModelLifecycle 按引擎链接发现选择。
 *
 * <p>English: On 1.20.1 Forge, CTM/Fusion/Athena render through the vanilla
 * BakedModel#getQuads path, so this wrapper only re-buckets UP/DOWN/null in both the 3-arg
 * (vanilla) and 5-arg (Forge ModelData/RenderType) getQuads and delegates every other
 ...
 * and is selected by ForgeModelLifecycle via engine linkage discovery.
 */
public final class GlassPaneSeamCulling {
    ...
    private static final class PaneCullingModel
        ...
        /**
         * 中文：Forge 5 参 getQuads 与 vanilla 3 参版本保持同样的 UP/DOWN/null 重分桶，
         * 并把 ModelData/RenderType 原样传给委托模型。
         *
         * English:
         * The Forge 5-arg getQuads keeps the same UP/DOWN/null re-bucketing as the vanilla
