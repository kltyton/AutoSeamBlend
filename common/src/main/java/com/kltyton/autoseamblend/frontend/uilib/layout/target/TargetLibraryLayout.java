...
public final class TargetLibraryLayout {
    ...
    private final TextBoxComponent searchInput =
            textBox("gui.autoseamblend.target.search");
    ...
    /** 中文：Screen 重建前冻结目标库列表的滚动位置。 / English: Freezes the target-list scroll position before a Screen rebuild. */
    public void captureScrollState() {
        listScrollOffset = listScroll != null
                ? listScroll.capture()
                : 0;
    }
    ...
    private ActionButton pickerButton(
        ...
        ActionButton button = new ActionButton(
            ...
            button.setActive(host.actionsEnabled());
        button.setAction(() -> {
            if (!pickerOpen) {
                pickerRequested.run();
            }
