...
/** 中文：用于隐式和配置 AutoBlend 表面的 Continuity 运行时实现。 / English: Continuity-backed runtime implementation for implicit and configured AutoBlend surfaces. */
public final class FabricContinuityAutoBlendProcessor implements QuadProcessor {
    ...
    private ProcessingResult processQuad(
        ...
        for (ContinuityOverlayOrchestrator.Candidate<FaceSurface> candidate : donors) {
            ...
            TextureAtlasSprite[] generated =
                    stateSprites.orElseThrow();
            // 中文：复用锁定 Continuity 公开 RenderUtil.findOverlayMaterial（javap 已证明
            // 签名），由其线程局部 MaterialFinder 设置 blendMode 与 AO；不复制引擎算法。
            // English: Reuse the locked Continuity public RenderUtil.findOverlayMaterial
            ...
            for (int slot : nativeSlots) {
                ...
                QuadUtil.emitOverlayQuad(
                        ...
                        tint,
                        cutoutMaterial);
                // 中文：1.21.1 的 ProcessingContextImpl.outputTo 仅在 hasExtraQuads=true 时把
                ...
                // English: In 1.21.1, ProcessingContextImpl.outputTo emits the extra-quad mesh
    ...
    /**
         * 中文：为工作台预览复用 Runtime 的同一个 Continuity overlay 选择器。
     *
     * English:
         * Reuses Runtime's exact Continuity overlay selector for the workbench
     * preview.
     */
    public static List<Integer> previewOverlaySlots(
        ...
        List<Integer> slots =
                selected.select(
                        FabricQuadEmitting.fromBakedQuad(RendererAccess.INSTANCE.getRenderer().meshBuilder().getEmitter(), 
                                query.surface()
                                        .representativeQuad()),
                        query.surface()
                                .sprite(),
