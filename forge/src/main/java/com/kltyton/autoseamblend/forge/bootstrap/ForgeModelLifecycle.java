...
import com.kltyton.autoseamblend.forge.compat.continuity.runtime.culling.ContinuityGlassPaneSeamCulling;
import com.kltyton.autoseamblend.forge.engine.registry.ForgeEngineRegistry;
import com.kltyton.autoseamblend.forge.runtime.culling.GlassPaneSeamCulling;
...
public final class ForgeModelLifecycle {
    private ForgeModelLifecycle() {}

    public static void onModifyBakingResult(ModelEvent.ModifyBakingResult event) {
        // 中文：Fusion 1.3.12 Forge 在 ModelEvent 之后才通过 ModelManager Mixin
        // 应用方块模型 modifier。此处捕获会早于原生模型与 accepted-document 发布，必须
        // 延迟到 Fusion modifier publish 钩子，否则整个代次会被标记为 UNKNOWN。
        // English: Fusion 1.3.12 Forge applies block model modifiers through its
        ...
        if (ForgeEngineRegistry.RUNTIME
                .linkableEngineIds()
                .contains("fusion")) {
            return;
        }
    ...
    private static void prepareModelFacts(
            Map<ResourceLocation, BakedModel> baked) {
        ...
        try {
            ...
            // getQuads，使用纯 Forge 包装器。选择只依据引擎链接发现，不使用 Gradle
            ...
            // render through vanilla getQuads and use the pure Forge wrapper. Selection
            ...
            // 中文：26.1.2 直接修改 event 自身的模型表；1.20.1 的 blockStateModels() 是副本，
            // 必须把安装好的玻璃板剔除包装器写回 event.getModels()，否则游戏实际渲染仍使用
            // 未包装模型，端盖剔除修复不会生效。
            // English: 26.1.2 mutates the event's own model map; 1.20.1 blockStateModels()
