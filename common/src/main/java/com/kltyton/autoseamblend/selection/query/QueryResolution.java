...
public record QueryResolution(
    ...
    public QueryResolution {
        ...
        if (!query.blockId().equals(key.targetId())
                || !query.stateProperties().equals(key.stateIdentity())
                || query.face() != key.face()
                || !query.spriteId().equals(key.spriteId())
                || query.requestedMethod() != key.requestedMethod()) {
        ...
        for (NativeOwnership claim : nativeAuthorExactClaims) {
            if (claim.match() != NativeOwnership.Match.MATCH
                    || claim.source()
                            .map(source -> source.tier() != SourceTier.NATIVE_AUTHOR)
                            .orElse(true)) {
                throw new IllegalArgumentException(
                        "native-author claims must be accepted tier-one exact matches");
            }
        }
        for (NativeOwnership claim : managedExactClaims) {
            if (claim.match() != NativeOwnership.Match.MATCH
                    || claim.source()
                            ...
                            .orElse(true)) {
                throw new IllegalArgumentException(
                        "managed claims must be accepted tier-two or tier-three matches");
            }
        }
        if (conservativeBlocker.isPresent()) {
            NativeOwnership blocker = conservativeBlocker.orElseThrow();
            if (!orderedContentClaims.contains(blocker)
