...
public final class TextureSourceSnapshot {
    ...
    public static TextureSourceSnapshot capture(
        ...
        int[] frames = contents.isAnimated()
    ...
    public static TextureSourceSnapshot capture(
            ...
            Identifier textureId) throws IOException {
        Objects.requireNonNull(resources, "resources");
        Objects.requireNonNull(textureId, "textureId");
        Identifier file = SpriteSource.TEXTURE_ID_CONVERTER
                .idToFile(textureId);
        ...
        try (var input = resource.open();
            ...
            int[] frames = animation
                    ...
                    .map(entries -> entries.stream()
                            .mapToInt(entry -> entry.index())
                            .distinct()
                            .toArray())
                    .filter(indices -> indices.length > 0)
                    .orElseGet(() -> sequentialFrames(
                            frameCount));
            validateFrames(
                    frames,
                    sheetWidth / frame.width(),
    ...
    /** 中文：首动画帧中的一个不可变矩形编辑。 / English: One immutable rectangular edit within the first animation frame. */
