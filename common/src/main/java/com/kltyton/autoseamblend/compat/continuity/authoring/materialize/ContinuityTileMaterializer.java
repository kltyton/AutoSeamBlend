...
public final class ContinuityTileMaterializer {
    ...
    public static MaterializedSheet materializeSheet(
        ...
        it.unimi.dsi.fastutil.ints.IntArrayList frameList =
                new it.unimi.dsi.fastutil.ints.IntArrayList();
        animation.ifPresent(value -> value.forEachFrame(
                (index, time) -> frameList.add(index)));
        int[] frameIndices;
        if (frameList.isEmpty()) {
            frameIndices = IntStream.range(0, frameCount).toArray();
        } else {
            frameIndices = frameList.stream()
                    .distinct()
