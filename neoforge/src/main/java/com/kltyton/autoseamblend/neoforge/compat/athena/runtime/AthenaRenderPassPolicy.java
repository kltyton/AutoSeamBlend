...
/**
 * 中文：Athena 渲染 pass 的纯策略：广告渲染类型与按 pass 分流。不访问 generation/引擎，
 * 只根据调用方提供的原始渲染类型、当前 renderType 与 needsOverlay 布尔做决定，便于
 * JUnit 直接锁定（ChunkRenderTypeSet 实现 Iterable&lt;RenderType&gt;，测试用 List 即可）。
 * 语义与 26.1.2 的 overlay 材质层（CUTOUT）一致：额外 overlay 只在 cutout pass 发射。
 *
 * <p>English: Pure Athena render-pass policy: advertised render types and per-pass routing.
 ...
 * The semantics match 26.1.2's overlay material layer (CUTOUT): extra overlays are emitted
 * only on the cutout pass.
 */
final class AthenaRenderPassPolicy {
    private AthenaRenderPassPolicy() {}
