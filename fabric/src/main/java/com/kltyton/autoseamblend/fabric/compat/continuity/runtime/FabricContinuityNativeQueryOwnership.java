...
/**
 * 中文：另一引擎适配器变换 Quad 前复用精确的已接受 Continuity 文档身份；探针就是已接受
 * holder 谓词，每个 holder 代次都会重建，绝不在渲染线程解析 properties。
 *
 * English: Exact accepted Continuity document identity reused before another
 * engine adapter transforms a quad. The probes are the accepted holder
 * predicates themselves and never parse properties on the render thread.
 */
public enum FabricContinuityNativeQueryOwnership
    ...
    public static void stageGeneration() {
        long generation;
        synchronized (
                FabricContinuityNativeQueryOwnership.class) {
            if (capturingGeneration < 0) {
                throw new IllegalStateException(
                        "Continuity ownership capture has not begun");
            }
    ...
    public NativeQueryObservation observe(
        ...
        MutableQuadView mutable =
                FabricQuadEmitting.fromBakedQuad(emitter, quad);
        ContinuityProcessingDataProbe data =
                new ContinuityProcessingDataProbe();
    ...
    @Override
    public boolean prepared(long generation) {
        synchronized (
                FabricContinuityNativeQueryOwnership.class) {
    ...
    private record Probe(
        ...
        private boolean owns(
            ...
            if (caching.affectsBlockStates()
                    && !caching.affectsBlockState(
                            state)) {
                return false;
            }
            if (caching.affectsSprites()
                    && !caching.affectsSprite(
                            sprite)) {
                return false;
            }
