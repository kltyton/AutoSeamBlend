...
public final class MinecraftNativeResourceSource implements NativeResourceSource {
    ...
    public Optional<byte[]> read(String resourceId) {
        ResourceLocation identifier = ResourceLocation.tryParse(resourceId);
        if (identifier == null) {
    ...
    public TextureResourceState inspectTexture(
        ...
        ResourceLocation file = textureFile.map(ResourceLocation::tryParse).orElse(null);
        ResourceLocation nativeSprite = ResourceLocation.tryParse(spriteId);
        if (file == null || nativeSprite == null) {
            return TextureResourceState.INVALID;
        }
        ResourceLocation nativeFile = SpriteSource.TEXTURE_ID_CONVERTER.idToFile(nativeSprite);
