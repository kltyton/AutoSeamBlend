...
/**
 * 中文：把预览场景的每个 node Quad 送入 Fabric Continuity 3.0.0+1.21 真实处理器链
 * （同一 QuadProcessors 切片与 ProcessingData 语义），不复制任何 CTM 算法；与
 * NeoForge 侧共享 common PreviewSceneQuadProcessor 契约，实现隔离在本 compat 包。
 *
 * English: Feeds every preview-scene node quad through Fabric Continuity
 * 3.0.0+1.21's real processor chain (the same QuadProcessors slice and
 * ProcessingData semantics), without copying any CTM algorithm; shares the
 * common PreviewSceneQuadProcessor contract with the NeoForge side while the
 * implementation stays isolated in this compat package.
 */
public final class FabricContinuityPreviewSceneQuadProcessor
        implements PreviewSceneQuadProcessor {
    private static final int MAX_PASSES = 16;

    public static final FabricContinuityPreviewSceneQuadProcessor INSTANCE =
            new FabricContinuityPreviewSceneQuadProcessor();

    private FabricContinuityPreviewSceneQuadProcessor() {}
    ...
    private static void processOne(
            ...
            long randomSeed,
            ArrayList<BakedQuad> output) {
        FabricContinuityPreviewProcessingContext context =
                new FabricContinuityPreviewProcessingContext();
        MutableQuadView quad = FabricQuadEmitting.fromBakedQuad(
    ...
    /**
     * 中文：按 Fabric Continuity ProcessingContextImpl.outputTo 的契约回放额外 Quad
     * （emitterConsumers → addMesh → meshBuilder.build），转成 BakedQuad 追加到输出；
     * 声明了额外 Quad 但零产出时保持输出不变，不产生日志。
     *
     * English: Replays extra quads following Fabric Continuity's
