...
public final class MinecraftSelectorResolver implements ConnectionRuleSet.Resolver<Block> {
    ...
    public boolean isValidId(String id) {
        return id.indexOf(':') > 0 && ResourceLocation.tryParse(id) != null;
    }
    ...
    public Set<Block> tag(String id) {
        ...
        net.minecraft.core.HolderLookup.RegistryLookup<Block> blockRegistry =
                registryAccess.lookupOrThrow(Registries.BLOCK);
        HashSet<Block> blocks = new HashSet<>();
        blockRegistry.get(tag).ifPresent(holders -> {
