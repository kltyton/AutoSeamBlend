...
public final class FusionModelLifecycleState {
    ...
    public synchronized Optional<BakedModel> baseModel(
            long tokenOrdinal,
            long generation,
            BlockState state) {
     ...
     */
    public synchronized Optional<BakedModel> previewModel(
            long generation,
            BlockState state) {
        ...
        BakedModel model = slot.previewModels().get(key);
        if (model == null) {
    ...
    private record BakeSlot(
            long generation,
            Map<BlockState, BakedModel> baseModels,
            Map<BlockState, BakedModel> previewModels) {
    }
}
