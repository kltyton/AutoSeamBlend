...
/**
 * 中文：Athena 原生模型所有权的 Loader 无关裁决策略，由 1.21.1 a71485e 已验收 common
 * 逻辑移植。只做三类纯决策：缺失纹理判定、候选精灵同名所有权、以及 owns + 文档身份到
 * 观察结果的裁决；不接触 Athena 模型、WrappedGetter 或纹理槽提取，那些仍属于各 Loader
 * 适配边界。
 *
 * <p>English: Loader-neutral adjudication policy for Athena native-model ownership, ported
 * from the accepted 1.21.1 a71485e common logic. It only makes three pure decisions:
 * missing-sprite detection, same-name candidate-sprite ownership, and the
 * owns-plus-document-identity to observation verdict; it never touches the Athena model,
 * WrappedGetter, or texture-slot extraction, which remain in each Loader's adapter
 * boundary.
 */
public final class AthenaNativeOwnershipPolicy {
     ...
     * 无法可靠解析→明确 unknown（绝不伪造 exact(empty)，该构造在 26.1.2 会抛异常）。
     *
     ...
     * exact(empty), which throws on 26.1.2).
     */
