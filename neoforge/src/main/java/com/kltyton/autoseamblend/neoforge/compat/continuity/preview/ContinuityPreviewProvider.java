...
enum ContinuityPreviewProvider
    ...
    private static PreviewSample sample(
        ...
        MutableQuad quad = new MutableQuad().setFrom(
    ...
    /**
     * 中文：预览查询使用的短生命周期原生处理数据，不跨帧共享可变引擎状态。
     *
     * English:
     * Short-lived native processing data for one preview query. Mutable engine
     * state is never shared across frames.
     */
    private static final class PreviewProcessingContext
        ...
        private final QuadCollectionBuilder
                extraQuads =
                        new QuadCollectionBuilder();
