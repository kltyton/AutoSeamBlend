 ...
 * 契约与 NeoContinuity 的 ProcessingContextImpl 一致：额外 Quad 累积在 meshBuilder，
 * 由 drainExtraQuads 在处理器链结束后回放到目标 emitter（等价 outputTo）。
 *
 * English: Short-lived native processing data shared by the preview scene and the
 * exact-face sampler, mirroring NeoContinuity's ProcessingContextImpl: extra quads
 * accumulate in the meshBuilder and drainExtraQuads replays them to a target
 * emitter after the processor chain (equivalent to outputTo). Mutable engine state
 * is never shared across frames.
 */
final class ContinuityPreviewProcessingContext
