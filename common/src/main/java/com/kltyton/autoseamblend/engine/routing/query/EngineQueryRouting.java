...
public final class EngineQueryRouting {
    ...
    public static EngineRouteSelection resolveExact(ExactInput input) {
        Objects.requireNonNull(input, "input");
        QueryResolution resolution = QueryArbiter.resolveRuntime(
                input.generation(),
                input.reloadToken(),
                ...
                input.engines());
        Optional<NativeOwnership> winningClaim = resolution.orderedContentClaims().isEmpty()
                ? Optional.empty()
                : Optional.of(resolution.orderedContentClaims().get(0));
    ...
    private static ConnectionMethod requestedMethod(
        ...
        for (NativeOwnership claim : ordered) {
        ...
        if (explicit.isPresent()) {
            return explicit.orElseThrow().method();
        }
        return implicit.map(SelectionIntent::method).orElse(ConnectionMethod.NONE);
    }

    private static EngineRouteProvenance provenance(QueryResolution resolution) {
        if (!resolution.orderedContentClaims().isEmpty()) {
            Optional<NativeRuleSource> source =
                    resolution.orderedContentClaims().get(0).source();
            if (source.isPresent()) {
                return EngineRouteProvenance.document(source.orElseThrow());
            }
        }
        return resolution.selection()
                .map(intent -> EngineRouteProvenance.config(intent.sourceTier(), intent.order()))
                .orElseGet(EngineRouteProvenance::stableFallback);
    }
