...
/** 中文：在追加的 AutoBlend 处理器运行前保留精确原生查询所有权。 / English: Preserves exact native query ownership before the appended AutoBlend processor runs. */
public final class OwnershipTrackingProcessor implements QuadProcessor {
    private final QuadProcessor delegate;
    private final ProcessingPredicate queryPredicate;
    private final ContinuityProcessorMetadata metadata;

    private OwnershipTrackingProcessor(
    ...
    public static QuadProcessors.ProcessorHolder wrap(
        ...
        ProcessingPredicate queryPredicate =
                predicate(processor);
        ContinuityProcessorMetadata metadata =
                ContinuityProcessorMetadata.take(
                        processor);
        ContinuityNativeQueryOwnership.register(
                holder.predicates(),
                queryPredicate,
                metadata);
        return new QuadProcessors.ProcessorHolder(
                new OwnershipTrackingProcessor(
                        processor,
                        queryPredicate,
                        metadata),
                holder.predicates());
    ...
    public ProcessingResult processQuad(
        ...
        boolean accepted = queryPredicate != null
                && queryPredicate.shouldProcessQuad(
                        ...
                        context);
        if (accepted) {
            NativeOwnershipTracker.claim(metadata);
            if (NativeOwnershipTracker.shouldSkip(metadata)) {
        ...
        java.util.List<Integer> selectedOverlaySlots =
                accepted
                        ...
                        : java.util.List.of();
        NativeOwnershipTracker.begin(
                metadata,
                selectedOverlaySlots);
        boolean mayCompleteMissingReplacement =
                accepted
                        && !metadata.additiveOverlay()
                        && !metadata.presentResourceFailedAtlas()
                        && metadata.nativeSlots()
                                .stream()
                                .anyMatch(slot ->
                                        slot.intent()
                                                == NativeSlotIntent.DECLARED_MISSING)
                        && NativeOwnershipTracker.allowsAutoBlend(
                                RuleRuntime.current(),
                                state.getBlock());
        BakedQuad original = mayCompleteMissingReplacement
        ...
        if (original != null
            ...
            quad.fromVanilla(
                    original,
                    QuadConversions.material(),
                    original.getDirection());
            return ProcessingResult.NEXT_PROCESSOR;
        }
        boolean ownsQuery =
                accepted || result != ProcessingResult.NEXT_PROCESSOR;
        if (ownsQuery) {
            if (!accepted) {
                NativeOwnershipTracker.claim(metadata);
            }
    ...
    private static SourceTier sourceTier(
            QuadProcessors.ProcessorHolder holder) {
        if (holder.processor()
                instanceof OwnershipTrackingProcessor tracking) {
    ...
    private static ProcessingPredicate predicate(QuadProcessor processor) {
        if (processor instanceof AbstractQuadProcessor) {
            return ((AbstractQuadProcessorAccessor) processor)
                    .autoseamblend$processingPredicate();
        }
        if (processor instanceof SimpleQuadProcessor) {
            return ((SimpleQuadProcessorAccessor) processor)
                    .autoseamblend$processingPredicate();
        }
        return null;
    }
