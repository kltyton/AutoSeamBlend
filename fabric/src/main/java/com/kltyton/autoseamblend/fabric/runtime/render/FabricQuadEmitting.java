...
/**
 * 中文：把 vanilla BakedQuad 经 FRAPI 1.20.1 的 fromVanilla 复制到 QuadEmitter 的共享入口；
 * 26.1 的 fromBakedQuad 在 1.20.1 中不存在。2 参数入口保留给 Athena（沿用默认材质与
 ...
 * 1.20.1's fromVanilla; 26.1's fromBakedQuad does not exist in 1.20.1. The 2-arg entry is
...
public final class FabricQuadEmitting {
    ...
    public static RenderMaterial cutoutMaterial() {
        return Materials.CUTOUT;
    }

    /**
     * Lazily initialized on the first emitted quad, after the renderer is available. Render
     * materials contain immutable pipeline flags and do not depend on atlas generations.
     */
    private static final class Materials {
        private static final RenderMaterial DEFAULT = RendererAccess.INSTANCE
                .getRenderer()
                .materialFinder()
                .find();
        private static final RenderMaterial CUTOUT = RendererAccess.INSTANCE
                .getRenderer()
                .materialFinder()
