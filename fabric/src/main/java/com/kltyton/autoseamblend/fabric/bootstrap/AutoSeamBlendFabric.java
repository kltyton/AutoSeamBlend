...
/**
 * 中文：1.20.1 Fabric 客户端 composition root。
 *
 * <p>English: 1.20.1 Fabric client composition root. Fabric has no CTM Mod
...
public final class AutoSeamBlendFabric implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        if (engines.engineRequired()) {
            for (EngineDiagnostic diagnostic :
                    engines.registry().diagnostics()) {
                if (diagnostic.severity()
                    ...
                    Constants.LOG.error(
                        ...
                        == EngineDiagnostic.Severity.WARNING) {
                    Constants.LOG.warn(
                            "{}: {} {}",
                            diagnostic.code(),
                            diagnostic.message(),
