...
/**
 * 中文：预览渲染所需的薄渲染状态；场景几何和值语义位于 common。
 *
 * English: Thin render state for the preview renderer; scene geometry and
 * value semantics live in common.
 */
        ...
        this(
                ...
                scissorArea == null
                        ? new ScreenRectangle(
                                request.x0(),
                                request.y0(),
                                request.x1() - request.x0(),
                                request.y1() - request.y0())
                        : scissorArea.intersection(
                                new ScreenRectangle(
                                        request.x0(),
                                        request.y0(),
                                        request.x1() - request.x0(),
