...
import com.kltyton.autoseamblend.forge.frontend.uilib.screen.ForgePreviewWorkspaceRenderer;
...
/**
 * 中文：Forge 仅组合原生端口与 common 工作台；会话、布局、控件和操作令牌均由 common 拥有。
 * English: Forge only composes its native port with the common workbench;
 * common owns the session, layout, widgets, and operation tokens.
 */
public final class UilibWorkbenchController {
    private final ForgeWorkbenchNativePort nativePort;
    ...
    private UilibWorkbenchController(
        ...
        nativePort = new ForgeWorkbenchNativePort(
                minecraft,
                selection.family(),
                selection.engineId());
        ...
        screen = new UilibWorkbenchScreen<>(
                session,
                new ForgePreviewWorkspaceRenderer<>(
    ...
    private static UilibWorkbenchScreen<ManagedAuthoringDraft> blockedScreen(
        ...
        return new UilibWorkbenchScreen<>(
                session,
                new ForgePreviewWorkspaceRenderer<>(ignored -> Optional.empty(), () -> {}),
