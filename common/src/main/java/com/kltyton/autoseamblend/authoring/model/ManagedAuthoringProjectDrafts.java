...
public final class ManagedAuthoringProjectDrafts {
    ...
    public static ManagedAuthoringRule createRule(
        ...
        List<String> preferredKeys = MinecraftModelTextureBindings.resolve(
                resources,
                draft.originalModelId(),
                draft.sourceTextureId());
        if (!preferredKeys.isEmpty()) {
            return createRule(draft, draft.originalModelId(), preferredKeys);
        }
        Optional<BlockstatePreferredModelResolution.Result> resolved =
                BlockstatePreferredModelResolution.resolve(
                        resources,
                        draft.targetBlockId(),
                        draft.sourceTextureId());
        if (resolved.isPresent()) {
            BlockstatePreferredModelResolution.Result result = resolved.orElseThrow();
            return createRule(
                    draft,
                    result.selectedModelId(),
                    result.sourceTextureKeys());
        }
    ...
    private static Block requireBlock(String targetBlockId) {
        ResourceLocation target = ResourceLocation.tryParse(
