...
/**
 * 中文：双 Loader 共享的 pane 表面角色选择组件，由 1.21.1 08d4bc5 已验收 common 逻辑
 * 移植并适配 26.1.2 Identifier/BakedQuad API。语义：body=水平轴主面（最大面积条带面）；
 ...
 * 包装不进入本组件。
 *
 * <p>English: Loader-shared pane surface-role selection component, ported from the accepted
 * 1.21.1 08d4bc5 common logic and adapted to the 26.1.2 Identifier/BakedQuad APIs. Semantics:
 * body is the horizontal main face (the largest strip face); edge prefers the vertical cap
 ...
 * non-edge-sprite filter. Purely data-driven with no block/sprite whitelists; typical inputs
 * (vertical faces all caps) stay value-identical to the accepted NeoForge selection.
 * Loader-specific first-bake collection and model wrapping never enter this component.
 */
public final class AthenaPaneSurfaceRoles {
    ...
    public static Optional<Roles> select(
        ...
        Identifier bodyCandidateSprite =
     ...
     * 时，按 1.21.1 0d5bce0 已验收的跨状态回退语义，只从同一 Block 的其他连接状态借用
     * 稳定 cap FaceSurface；body 始终保持当前 state。无可用 sibling 时明确安全回退（返回
     * 当前退化角色，不抛异常、不空）。纯数据驱动，不依赖方块 ID/精灵名白名单。
     *
     ...
     * another connection state of the same Block per the accepted 1.21.1 0d5bce0 cross-state
